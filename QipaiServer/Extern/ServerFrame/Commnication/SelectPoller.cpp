...
#define SELECT_ERROR(obj, iter, pollset) {\
    obj->onError();\
        ...
        while (eventCounts > 0 && iter != m_PollMap.end())
        {
            ...
            do
            {
                ...
                if (FD_ISSET(cid, &inset))
                {
                    ...
                    {
                        isError = true;
						SELECT_ERROR(obj, iter, m_PollMap);
                    }
                }
...
{
    if (obj != nullptr && m_PollMap.find(obj->getCommId()) == m_PollMap.end())
        return -1;
...
IComm* SelectPoller::getComm(COMMID cid)
{
...
{
    for (set<IComm*>::iterator iter = m_RemoveSet.begin(); iter != m_RemoveSet.end(); ++iter)
    {
        IComm* obj = *iter;
        COMMID cid = obj->getCommId();
        FD_CLR(cid, &m_InSet);
