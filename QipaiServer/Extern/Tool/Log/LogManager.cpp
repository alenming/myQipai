...
{
    for (map<int, ILogHandler*>::iterator iter = m_Handlers.begin(); iter != m_Handlers.end(); ++iter)
    {
        iter->second->onExit();
        delete iter->second;
    }
...
void LogManager::writelog(int tag, int level, const char* file, const char* func, int line, const char* log, ...)
{
    ...
    if (level >= m_LogLevel)
    {
        ...
        {
            if (iter->second->getTag() & tag)             // 与为ture的才执行
            {
                if (1 == tag)
                    iter->second->onLog(level, m_StringFormat);
                else    // tag 如果不是默认值1的话, 自己处理前缀格式
...
//				m_StringFormat += "[Info]";
    ...
    if (m_DateOption || m_TimeOption)
    {
        ...
        if (m_DateOption)
        {
            sprintf_s(szBuf, sizeof(szBuf), "[%04d:%02d:%02d]", tmnow.tm_year + 1900, tmnow.tm_mon, tmnow.tm_mday);
			m_StringFormat += szBuf;
//			m_StringFormat += __DATE__;
//			m_StringFormat += " ";
        }

        if (m_TimeOption)
        {
            sprintf_s(szBuf, sizeof(szBuf), "[%02d:%02d:%02d]", tmnow.tm_hour, tmnow.tm_min, tmnow.tm_sec);
            m_StringFormat += szBuf;
//			m_StringFormat += __TIME__;
        }
    }
    if (m_FileOption && nullptr != file)
		m_StringFormat += "[" + string(file) + "] ";
    if (m_FuncOption && nullptr != func)
        m_StringFormat += "[" + string(func) + "] ";
    if (m_LineOption)
    {
